...
static uint32_t CLOCK_GetMstrOscFreq(void)
{
    ...
    switch ((clock_mstr_osc_clk_src_t)u8MstrOscClkSrc)
    {
        case kCLOCK_MstrOscClkSrcFircDiv6:
            u32MstrOsc = CLOCK_GetFastIrcFreq() / 6U;
            break;
...
uint32_t CLOCK_GetFreq(clock_name_t eClkName)
{
    ...
    switch (eClkName)
    {
        ...
        case kCLOCK_FastIrcDiv6Clk:
            u32Freq = CLOCK_GetFastIrcFreq() / 6U;
            break;
    ...
    if (psConfig->bPllEnable)
    {
        ...
        switch (psConfig->eMstrOscClkSrc)
        {
            case kCLOCK_MstrOscClkSrcExt:
                ...
                break;
            case kCLOCK_MstrOscClkSrcFircDiv6:
                u32MstrOscClkFreq = CLOCK_GetFastIrcFreq() / 6U;
                break;
            default:
...
void CLOCK_EnableUsbfs0Clock(void)
{
    ...
    while (0U != (USB->CLK_RECOVER_INT_STATUS & USB_CLK_RECOVER_INT_STATUS_OVF_ERROR_MASK))
    {
