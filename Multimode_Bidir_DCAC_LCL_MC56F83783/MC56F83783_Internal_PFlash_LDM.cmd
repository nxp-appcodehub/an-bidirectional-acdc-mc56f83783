...
##
##  5. To use .x_FlashData memory segment, when code are compiled in LDM mode, please uncheck below setting:
##     Project Properties -> C/C++ Build -> Settings -> Tool Settings -> DSC Compiler -> Processor -> Globals Live Lower Memory.
##
##    ***************************************************************
...
MEMORY {
  ...
  .p_Interrupts (RWX): ORIGIN = 0x0,     LENGTH = 0xE0             # Interrupt Vector Table
  ...
  .p_Code       (RWX): ORIGIN = 0x208,   LENGTH = 0x20000 - 0x208 - 0x400  # XIP(excute in place) code resides in flash
  .x_FlashData   (RX): ORIGIN = AFTER(.p_Code) + 0x20000, LENGTH = 0  # Flash data stored in flash but accessed from x space

  .p_RamCode     (RX): ORIGIN = 0x60000, LENGTH = 0x8000           # RAM mapped into p space, code resides in flash and runs in RAM

  .p_RamData     (RX): ORIGIN = AFTER(.p_RamCode) - 0x60000, LENGTH = 0x8000 + 0x60000 - AFTER(.p_RamCode)  # .data section
  .x_Data        (RW): ORIGIN = AFTER(.p_RamCode) - 0x60000, LENGTH = 0x8000 + 0x60000 - AFTER(.p_RamCode)  # bss, heap, stack ...

  .p_NVM        (RWX): ORIGIN = 0x20000 - 0x400, LENGTH = 0x400    # Use flash last sector as NVM, stored and accessed in p space
}
